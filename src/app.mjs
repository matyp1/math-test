...
function openGame(nextMode,replay=false,id=null) {
  if(nextMode==='level'){
    ...
    if(!target || !canPlay(progress,levels,target.id))return;
    level=target;
  }
  mode=nextMode;
  if(replay){boards[mode==='tutorial'?'tutorial':level.id]=createBoard(puzzles());if(mode==='tutorial'){progress.tutorialStarted=false;progress.tutorialBiteSelected=false;boards.tutorial.pool=['dental-courage:2','dental-courage:0'];}}
  undoBoard=null;board().selected=null;collapsed=false;save();navigate('game');
}
...
function home() {
  ...
  return `<section class="screen home"><header class="home-header">${brand('home-brand')}${stars()}<button class="icon-button" data-action="help" aria-label="How to play">?</button></header><div class="home-greeting">${quip('explain')}<div><p class="eyebrow">${count} / ${levels.length} LEVELS COMPLETE</p><h1 tabindex="-1">Good words.<br><em>Busy brain.</em></h1></div></div><button class="level-card" data-level="${next.id}" data-replay="${finished}"><span class="eyebrow">LEVEL ${next.number} · ${next.tag}</span><strong>${finished?'All ten. All tidy.':next.title}</strong><span>${finished?'Fifty phrases untangled. Fancy another round?':next.intro}</span><span class="card-foot">${finished?'Replay the grand untangle':filledCount(boards[next.id],next.puzzles)?'Continue where you left off':`Play Level ${next.number}`} →</span></button><section class="level-journey" aria-label="Choose a level"><h2>Your ten-level journey</h2><p>Five phrases each. A little more tangled every time.</p><div class="level-list">${levels.map(l=>{const done=progress.completedLevels.includes(l.id),locked=!canPlay(progress,levels,l.id),started=filledCount(boards[l.id],l.puzzles)>0;return `<button class="journey-level ${done?'done':''}" data-level="${l.id}" data-replay="${done}" ${locked?'disabled':''} aria-label="Level ${l.number}: ${l.title}. ${locked?`Locked. Complete Level ${l.number-1} first`:done?'Completed. Replay':started?'Continue':'Play'}"><b>${l.number}</b><span><strong>${l.title}</strong><small>${locked?`Complete Level ${l.number-1} to unlock`:done?'Complete · replay without extra reward':started?'In progress · resume your board':l.tag}</small></span><span aria-hidden="true">${locked?'🔒':done?'✓':'→'}</span></button>`;}).join('')}</div></section><button class="tutorial-card" data-action="${progress.tutorialComplete?'replay-tutorial':'tutorial'}"><span class="tutorial-icon">1</span><span><strong>ONE SLOP</strong><small>${progress.tutorialComplete?'Replay the hands-on tutorial':'Learn to lift, place and check'}</small></span><b>→</b></button><div class="home-note"><span>★</span><p><strong>A nudge when you need it.</strong><br>Extra clues cost 1 star. Each new level earns 1 star. Wrong guesses cost nothing.</p></div>${!storageAvailable?'<p class="storage-note">Your browser isn’t saving progress. Keep this tab open to keep playing.</p>':''}</section>`;
}
...
function poolMarkup() {
  ...
  return `<section class="pile ${collapsed?'collapsed':''}" aria-label="The Slop Pile"><div class="pile-heading"><div><h2><span aria-hidden="true">∞</span> THE SLOP PILE</h2><p>${b.selected?`<strong>${b.tokens[b.selected].word}</strong> lifted · tap a space`:'Tap a word, then tap a space.'}</p></div><button class="pile-toggle" data-action="toggle-pile" aria-expanded="${!collapsed}" aria-label="${collapsed?'Expand':'Collapse'} word pile"><b>${b.pool.length}</b><span>left ${collapsed?'⌃':'⌄'}</span></button></div><div class="pool-words" role="group" aria-label="Available words">${b.pool.map((id,index)=>`<button class="word pool-word ${b.selected===id?'selected':''} ${tutorialLocked&&b.tokens[id].word==='BITE'?'teach-word':''}" data-pool="${escape(id)}" data-token="${escape(id)}" data-focus="pool-${escape(id)}" aria-label="Lift ${b.tokens[id].word}" aria-pressed="${b.selected===id}" ${tutorialLocked&&b.tokens[id].word!=='BITE'?'disabled':''} style="--tilt:${((index%5)-2)*0.65}deg">${b.tokens[id].word}</button>`).join('')||'<p class="empty-pile">Every word has a home. Happy with them? Check the slop.</p>'}</div><div class="selection-tools">${undoBoard?'<button class="text-button undo-word" data-action="undo" aria-label="Undo last move">↶ Undo</button>':''}${canReturn?'<button class="text-button return-word" data-action="return">↙ Return to pile</button>':'<span>Words can move between any lanes.</span>'}${b.selected?'<button class="text-button" data-action="deselect">Deselect</button>':''}</div></section>`;
}
function game() {
  const tut=mode==='tutorial',instruction=tut?tutorialInstruction():{title:'Five slops. One big mess.',text:level.intro,pose:'thinking'},intro=tut&&!progress.tutorialStarted,filled=filledCount(board(),puzzles()),total=Object.keys(board().tokens).length;
  return `<section class="screen game ${tut?'tutorial':`five-slops ${level.number>=2?'independent-play':''}`}">${topbar()}<div class="game-title"><div><p class="eyebrow">${tut?'LEARN THE ROPES':`LEVEL ${level.number} · ${level.tag}`}</p><h1 tabindex="-1">${tut?'ONE SLOP':'FIVE SLOPS'}</h1></div><span class="progress-badge">${tut?`${instruction.step||1} / 3`:`${filled} / ${total}`}<small>${tut?'TUTORIAL':'WORDS PLACED'}</small></span></div><div class="coach ${!tut&&level.number>=2?'quiet-coach':''} ${wrong?'wrong-feedback':''}">${quip(wrong?'react':instruction.pose)}<div class="speech"><strong>${wrong?'Still a little sloppy.':instruction.title}</strong><p>${escape(feedback||instruction.text)}</p></div></div>${intro?`<div class="tutorial-intro"><p>Start with <strong>BITE THE BULLET.</strong><br>THE is already in place. You do the rest.</p><p class="star-intro">★ You start with ${progress.stars} ${progress.stars===1?'star':'stars'}. Use stars for extra clues in Five Slops.</p><button class="button primary" data-action="begin-tutorial">Let’s go <span>→</span></button></div>`:`<div class="board-content"><div class="board-legend">${lockIcon}<span>Locked words stay put.</span>${!tut?'<span class="legend-right">5 phrases · one pile</span>':''}</div><div class="lanes" aria-label="${tut?'One phrase':'Five phrase lanes'}">${puzzles().map(laneMarkup).join('')}</div>${tut?`<div class="tutorial-instruction"><strong>${instruction.title}</strong><span>${filled===total?'Nothing submits until you press the button.':'Tap a placed word to move it again.'}</span></div>`:''}</div><div class="play-dock">${feedback?`<p class="dock-feedback">${escape(feedback)}</p>`:''}${poolMarkup()}<div class="check-row"><button class="button primary check ${filled===total?'ready':''}" data-action="check">CHECK THE SLOP <span>→</span></button><p>${tut?'You choose when your answer is ready.':'All five phrases must be correct. No penalties.'}</p></div></div>`}${!storageAvailable?'<p class="storage-note">Progress isn’t saving in this browser.</p>':''}</section>`;
}
function success() {
  const tut=mode==='tutorial';
  return `<section class="screen success">${topbar()}<div class="success-art"><span class="spark spark-one" aria-hidden="true">✦</span>${quip('celebrate')}<span class="spark spark-two" aria-hidden="true">★</span></div><p class="eyebrow">${tut?'ONE SLOP, SORTED.':`LEVEL ${level.number} COMPLETE`} </p><h1 tabindex="-1">${tut?'Nice swap!':level.number===levels.length?'All ten. Untangled!':'Clean slop!'}</h1><p class="success-line">${tut?'You solved it. On your terms.':'Five messes. One very tidy brain.'}</p><div class="solved-phrases">${puzzles().map(p=>`<div class="solved-phrase"><span>${p.icon}</span><p>${p.solution.join(' ')}</p><b aria-label="Correct">✓</b></div>`).join('')}</div><div class="reward-card"><span class="reward-star">★</span><div><strong>${lastReward?'+1 star':'Already rewarded'}</strong><p>${lastReward?'First-time solve reward':'Same satisfaction. No extra stars.'}</p></div>${quip('success')}</div><div class="success-actions">${tut?'<p><strong>Same idea. Bigger mess.</strong><br>Five phrases share one pile. Move words anywhere.</p><button class="button primary" data-action="level">Unleash Five Slops <span>→</span></button>':`${level.number<levels.length?`<button class="button primary" data-level="${levels[level.number].id}">Play Level ${level.number+1} <span>→</span></button>`:'<p class="campaign-complete">All 50 phrases restored. You’ve mastered this mess.</p>'}<button class="button secondary" data-action="home">${level.number===levels.length?'Your completed journey':'Choose a level'}</button><button class="text-button" data-level="${level.id}" data-replay="true">Replay Level ${level.number}</button>`}</div></section>`;
}
function render() {
...
function moveAndRender(change){
  const before=tileRects(),snapshot=structuredClone(board()),movingId=board().selected,changed=change();
  if(changed){snapshot.selected=null;undoBoard=snapshot;}
  if(changed){feedback='';wrong=false;vibrate(12);}save();render();
  if(changed){const placed=Array.from(app.querySelectorAll('[data-token]')).find(el=>el.dataset.token===movingId);placed?.classList.add('just-placed');}
...
function check(){
  ...
  lastReward=mode==='tutorial'?rewardOnce(progress,'tutorial-one'):completeLevel(progress,levels,level);
...
app.addEventListener('click',event=>{
  ...
  if(action==='replay-level'||action==='replay-tutorial'){openGame(action.slice(7),true,mode==='level'?level.id:null);return;}
  ...
  if(action==='undo'&&undoBoard){boards[mode==='tutorial'?'tutorial':level.id]=undoBoard;undoBoard=null;wrong=false;feedback='Last move undone.';save();render();announce(feedback);return;}
...
async function boot(){
  ...
  levels=await Promise.all(manifest.levels.map(async meta=>{const r=await fetch(new URL(`../data/${meta.file}`,import.meta.url));if(!r.ok)throw Error('Level could not load');return {...await r.json(),...meta};}));
  level=recommendedLevel(progress,levels);tutorialPuzzles=[levels[0].puzzles[0]];
